...
class GraphUserSendmailCommand extends GraphCommand {
  public get name(): string {
    return `${commands.USER_SENDMAIL}`;
  }
  ...
  public commandHelp(args: {}, log: (help: string) => void): void {
    ...
    Send an HTML e-mail to the specified e-mail addresses
      ${chalk.grey(config.delimiter)} ${this.name} --to chris@contoso.com,brian@contoso.com --subject 'DG2000 Data Sheets' --bodyContents 'The latest data sheets are in the <a href="https://contoso.sharepoint.com/sites/marketing">team site</a>' --bodyContentType HTML

    Send an HTML e-mail to the specified e-mail address loading e-mail contents
    from a file on disk
      ${chalk.grey(config.delimiter)} ${this.name} --to chris@contoso.com --subject 'DG2000 Data Sheets' --bodyContentsFilePath email.html --bodyContentType HTML
  
    Send a text e-mail to the specified e-mail address. Don't store the e-mail
    in sent items
      ${chalk.grey(config.delimiter)} ${this.name} --to chris@contoso.com --subject 'DG2000 Data Sheets' --bodyContents 'The latest data sheets are in the team site' --saveToSentItems false
`);
  }
}

module.exports = new GraphUserSendmailCommand();