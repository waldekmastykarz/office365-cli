...
describe(commands.APP_DEPLOY, () => {
  ...
  it('correctly handles random API error when site app catalog', (done) => {
    ...
    sinon.stub(request, 'post').callsFake((opts) => {
      ...
      if (opts.url.indexOf(`https://contoso.sharepoint.com/sites/apps/_api/web/sitecollectionappcatalog/AvailableApps/GetById('b2307a39-e878-458b-bc90-03bc578531d6')/deploy`) > -1) {
        if (opts.headers.accept &&
          opts.headers.accept.indexOf('application/json') === 0) {
          return Promise.reject({ error: 'An error has occurred' });
        }
