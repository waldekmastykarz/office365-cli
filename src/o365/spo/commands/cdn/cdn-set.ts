...
class SpoCdnSetCommand extends SpoCommand {
  ...
  Examples:
    ...
    Enable the Office 365 Public CDN on the current tenant
      ${chalk.grey(config.delimiter)} ${commands.CDN_SET} --type Public --enabled true

    Disable the Office 365 Public CDN on the current tenant
      ${chalk.grey(config.delimiter)} ${commands.CDN_SET} --type Public --enabled false

    Enable the Office 365 Private CDN on the current tenant
      ${chalk.grey(config.delimiter)} ${commands.CDN_SET} --type Private --enabled true
  
    Enable the Office 365 Private and Public CDN on the current tenant with
    default origins 
      ${chalk.grey(config.delimiter)} ${commands.CDN_SET} --type Both --enabled true 

    Enable the Office 365 Private and Public CDN on the current tenant without
    the default origins 
      ${chalk.grey(config.delimiter)} ${commands.CDN_SET} --type Both --enabled true --noDefaultOrigins
